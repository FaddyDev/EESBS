...
if(isset($_POST['edit'])){
    ...
    try {
   if(!($_FILES['logo']['size'] == 0) && !(empty($_POST['password']))) {
        $hashed_pass = password_hash($_POST['password'], PASSWORD_DEFAULT);
        $ext = end(explode(".", $_FILES["logo"]["name"]));
        move_uploaded_file($_FILES["logo"]["tmp_name"],"../images/logos/" . date('dmYhi').'_'.$_POST["cname"].'_logo.'.$ext);
			
			$location="images/logos/" . date('dmYhi').'_'.$_POST["cname"].'_logo.'.$ext;
            $logo = $location; $pass = $hashed_pass;     
    }
    else if(!($_FILES['logo']['size'] == 0) && empty($_POST['password'])) {
        $ext = end(explode(".", $_FILES["logo"]["name"]));
        move_uploaded_file($_FILES["logo"]["tmp_name"],"../images/logos/" . date('dmYhi').'_'.$_POST["cname"].'_logo.'.$ext);
			
			$location="images/logos/" . date('dmYhi').'_'.$_POST["cname"].'_logo.'.$ext;
            $logo = $location; $pass = $oldpass;
    }
    else if(($_FILES['logo']['size'] == 0) && !(empty($_POST['password']))) {
        $hashed_pass = password_hash($_POST['password'], PASSWORD_DEFAULT); 
        $logo = $oldlogo; $pass = $hashed_pass;
    }
    else { //if(($_FILES['logo']['size'] == 0) && empty($_POST['password']))
        $logo = $oldlogo; $pass = $oldpass;
    }

  $sql = "UPDATE s_providers SET logo=:logo, companyName=:cname, c_location=:loc, email=:email, phone=:phone, till=:till, username=:username, password=:pass WHERE uid=:uid";
    // Prepare statement
	...
	$stmt -> bindParam(":till", $_POST['till']);
...
<form class="panel panel-default" role="form" action="editprof.php" onsubmit="return confirmPass();" class="form-group" method="post" enctype="multipart/form-data">
<input type="hidden" name="oldlogo" value="<?php echo $data['logo'];?>"/>
<input type="hidden" name="uid" value="<?php echo $_SESSION['uid'];?>"/>
<?php if($data['logo']){ ?><center><img src="<?php echo '../'.$data['logo']; ?>" class="img-circle" height="100px" width="100px" alt="<?php echo $data['companyName'].' logo';?>" /></center> <?php } ?>
<div class="form-group well well-sm">
<label for="photo">Upload a photo of the company's logo <em>(Maximum size is 2MB)</em></label>
<br/><input type="file" name="logo" id="photo" title="choose and upload your photo" accept="image/jpeg,image/x-png,.jpg,.png" class="form-control add-todo">
</div>
<script type="text/javascript">
$('#photo').bind('change', function(){
  if(this.files[0].size > 1024*1024*2){
     alert('Maximum allowed size is 2MB; Your photo is '+this.files[0].size/1024/1024 + " MB");
     $(this).val('');;
     //return false;
  }
  else {return true;}
...
<input type="text" name="cname" value="<?php echo $data['companyName'];?>" class="form-control add-todo" id="log" placeholder="Enter the company/group name"  required />
</div>
<div class="form-group well well-sm">
<label for="location">Company Location</label>
<input type="text" name="loc" value="<?php echo $data['c_location'];?>" class="form-control add-todo" id="log" placeholder="Enter the company/group base"  required />
</div>
<div class="form-group well well-sm">
<label for="email">Email</label>
<input type="email" name="email" value="<?php echo $data['email'];?>" class="form-control add-todo" id="log" placeholder="Enter the company/group email"  required />
</div>
<div class="form-group well well-sm">
<label for="phone">Phone Number</label>
<input type="text" name="phone" value="<?php echo $data['phone'];?>" class="form-control add-todo" id="log" onKeyPress="return numbersonly(event)" placeholder="Enter the company/group phone number"  required />
</div>
<div class="form-group well well-sm">
<label for="till">Till Number</label>
<input type="text" name="till" value="<?php echo $data['till'];?>" class="form-control add-todo" id="log" onKeyPress="return numbersonly(event)" placeholder="Enter the company/group Mpesa till number"  required />
</div>
<div class="form-group well well-sm">
